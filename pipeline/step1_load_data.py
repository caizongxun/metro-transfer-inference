...
import config
from data_adapter.public_od_adapter import load_public_od
...
def run() -> dict:
    ...
    if config.DATA_SOURCE == 'private':
        ...
        from data_adapter.private_afc_adapter import load_private_afc
        from data_adapter.public_od_adapter import aggregate_to_typical_day
        raw = load_private_afc(config.PRIVATE_AFC_PATH)
        typical = aggregate_to_typical_day(raw)
        return {'typical_od': typical, 'data_source': 'private'}
    else:
        print('[Step1] 使用公開 OD 資料')
        typical_od = load_public_od(
        ...
        return {'typical_od': typical_od, 'data_source': 'public'}
...
if __name__ == '__main__':
    ...
    print(f'\n流量最高前 10 個 OD 對：')
    print(df.groupby(['origin', 'destination'])['trips'].sum().nlargest(10))
